...
class BasicVSRPlusNet(BaseModule):
    ...
    def compute_refill_features(self, lrs, keyframe_idx):
        ...
        for i in keyframe_idx:
            for j in self.padding:
                step = self.padding - j
                intra_feats.append(self.edvr(lrs[:, i+j:i + num_frames:step].contiguous()))
            intra_feats = torch.stack(intra_feats, dim=1)
            feats_refill[i] = TSAFusion(
                mid_channels=self.mid_channels,
                num_frames=self.padding,
                center_frame_idx=self.padding)(intra_feats)
        return feats_refill
